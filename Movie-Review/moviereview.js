...
function filterItems(e) {
    ...
    const items = document.querySelectorAll(".movie-title")
...
const movieTitle = document.querySelector(".review-movie-title")
const movie = document.querySelector(".review-movie")
const movieReview = document.querySelector(".review-paragraph")

function addReview() {
    const newTitle = movieTitle.value.trim()
    const newMovie = movie.value.trim()
    const newReview = movieReview.value.trim()
    ...
    if (newTitle === "") {
        ...
        alert("Please enter your movie's main actors!")
        return
    }
    else if (newReview === "") {
        alert("Please enter your movie review!")
        return
    }
    else {
        ...
        div.innerHTML =
            ...
            `<h1 class="movie-title">${newTitle}</h1>
            <h2 class="movie">${newMovie}</h2>
            <div class="star">
                ${[1, 2, 3, 4, 5].map(i =>
    ...
    form.innerHTML = 
    `<div class="addreview">
            <p>Movie Title: <input type="text" class="review-movie-title"></p>

            <p>Actor: <input type="text" class="review-movie"></p>
...
function getItemsFromStorage() {
    let cards = localStorage.getItem("movie-cards");
...
function addCardToStorage() {
    ...
    localStorage.setItem('movie-cards', JSON.stringify(cards));
}
...
function loadFromLocalStorage() {
    ...
    cards.forEach(card => {
        ...
        div.classList.add("feature-card");
        div.innerHTML = `
            <h1 class="movie-title">${card.title}</h1>
            <h2 class="movie">${card.movie}</h2>
            <div class="star">
                ${[1, 2, 3, 4, 5].map(i =>
